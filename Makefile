...
.PHONY: test test-backend test-mcp test-frontend test-e2e test-ci

test: test-backend test-mcp test-frontend

test-backend:
	cd backend && uv sync --all-groups
	cd backend && $(BACKEND_TEST_ENV) .venv/bin/pytest

# The MCP server needs no database, Redis or auth secrets.
test-mcp:
	cd mcp && uv sync --all-groups
...
test-ci: test-backend test-mcp test-frontend test-e2e
